...
//			+ "WHERE (c.startAt <= :startAt and c.endAt <= :endAt)" // (행사 시작) ~ 조회 시작 날짜 ~ (행사 종료) ~ 조회 마지막 날짜인 경우
//			+ "OR (c.startAt >= :startAt and c.endAt <= :endAt)" // 조회 시작 날짜 ~ (행사 시작 ~ 행사 종료) ~ 조회 마지막 날짜인 경우
//			+ "OR (c.startAt >= :startAt and c.endAt >= :endAt)" // 조회 시작 날짜 ~ (행사 시작) ~ 조회 마지막 날짜 ~ (행사 종료)인 경우
//			+ "OR (c.startAt <= :startAt and c.endAt >= :endAt)" // (행사 시작) ~ 조회 시작 날짜 ~ 조회 마지막 날짜 ~ (행사 종료)인 경우
