...
public interface AttendRepository extends JpaRepository<AttendEntity, Long> {
	...
	@Query(
			"SELECT a FROM AttendEntity a WHERE a.memberId = :memberId AND a.isDeleted = false AND a.createdDate >= :startDate AND a.createdDate <= :endDate ORDER BY a.createdDate DESC")
