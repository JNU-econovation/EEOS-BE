...
class SecurityFilterChainTest {
    ...
    class NonAuthorizedEndpoints {
        ...
        void loginShouldReturn200() throws Exception {
            // given
            given(authService.authenticate(id, password))
    ...
    class UserEndpoints {
        ...
        void setAccessToken(){
            ...
            given(tokenResolver.getUserDataByAccessToken(VALID_JWT))
                    .willReturn(1L);
        ...
        void 일반유저_올바른_토큰이_있으면_200응답() throws Exception {
            mockMvc.perform(get("/api/members?activeStatus=all")
        ...
        void 일반유저_토큰으로_행사_생성_API_접근시_401응답_2() throws Exception {
            mockMvc.perform(delete("/api/programs")
                            .header(HttpHeaders.AUTHORIZATION, bearerToken()))
                    .andExpect(status().isForbidden());
        }
        ...
        void 일반유저_토큰으로_행사_생성_API_접근시_401응답_4() throws Exception {
            mockMvc.perform(put("/api/members/activeStatus/1")
                            .header(HttpHeaders.AUTHORIZATION, bearerToken()))
                    .andExpect(status().isForbidden());
        }
